...
if not mOldMethod then
    ...
    for i = 1, #angleTable do
        sortedTable[i] = { angle = angleTable[i], evaluation = evaluationTable[i], usage = usageTable[i] }
    end
    ...
    for i = 1, #sortedTable do
        if sortedTable[i].evaluation == highestRating then
            table.insert(highestRated, sortedTable[i])
        else
            break
        end
    end

    -- choose a random angle from the highest rated angles, taking into account usage
    local randomIndex = math.random(1, #highestRated)
    local randomValue = highestRated[randomIndex].angle
    highestRated[randomIndex].usage = highestRated[randomIndex].usage + 1

    -- update the evaluation of the selected angle based on usage
    if Got_Hit == true and HitHead == false then
        for i = 1, #angleTable do
            if angleTable[i] == randomValue then
                evaluationTable[i] = evaluationTable[i] - (0.1 * highestRated[randomIndex].usage)
                break
            end
        end
    elseif HitHead == true then
        for i = 1, #angleTable do
            if angleTable[i] == randomValue then
                evaluationTable[i] = evaluationTable[i] - (0.2 * highestRated[randomIndex].usage)
                break
            end
        end
    end

    -- reset usage if all angles have been used the same number of times
    local usageValues = {}
    for i = 1, #highestRated do
        table.insert(usageValues, highestRated[i].usage)
    end
    local maxUsage = math.max(#usageValues)
    local minUsage = math.min(usageValues[1])
    if maxUsage - minUsage >= 2 then
        for i = 1, #highestRated do
            if highestRated[i].usage == minUsage then
                highestRated[i].usage = 0
            end
        end
    end
